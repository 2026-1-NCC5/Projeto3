...
def confirm_camera_batch():
    ...
    for item in batch["items"]:
        ...
        weight_kg = float(item.get("weight_kg", 0.0)) / quantity if quantity > 0 else 0.0
        for _ in range(quantity):
            database.add_food_item(item["name"], None, confidence, user_id, weight_kg)
...
def list_items():
    ...
    items = database.get_all_food_items(user_id) if database else []
...
def add_item_manual():
    ...
    if db_id: return jsonify({"status": "success", "id": db_id})
...
def update_item():
    ...
    if not user_id: return jsonify({"error": "Unauthorized"}), 401
    data = request.json
    success = database.update_food_item(
        item_id=data.get('id'),
        name=data.get('name'),
        weight_kg=float(data.get('weight_kg', 0.0)),
        confidence=float(data.get('confidence', 1.0))
...
def delete_item_db(item_id):
    user_id = session.get('user_id')
    if not user_id: return jsonify({"error": "Unauthorized"}), 401
    success = database.delete_food_item(item_id)
    if success: return jsonify({"status": "success"})
